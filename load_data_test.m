...
p = [SkyRed, SkyGreen, SkyBlue, Temperature, Pressure, PM2_5]'; % PM_1,PM2_5,PM10
...
xlabel('Actual PM 10 levels','fontsize',20);
ylabel('Estimated PM 10 levels','fontsize',20);
title('Scatter Diagram for fitrensemble','fontsize',25);
xlim([0 max(Y)])
ylim([0 max(Y)])
...
h.XTickLabelRotation = 45;
title('Predictor Importance Estimates for TreeBagger');
xlabel('Estimates with Curvature Tests');ylabel('Predictors');
set(gca,'FontSize',20); set(gca,'TickDir','out'); set(gca,'LineWidth',2);
...
%%
ypred = predict(Mdl_TB,Xtest);
%%
plot(Ytest,'ro','MarkerSize',6)
hold on
plot(Ypred,'b--','LineWidth',2)
xlim([4500 6000])
%ylim([0 max(Y)])
%scatter(Xtest,Ypred,'gs','filled')
%plot(yint(:,1),"k:")
%plot(yint(:,2),"k:")
hold off
...
figure;
bar(imp);
title('Predictor Importance Estimates Binary Decision Tree');
ylabel('Estimates');
xlabel('Predictors');
h = gca;
% Set default font sizes and other properties
...
%% 
%p = [SkyRed, SkyGreen, SkyBlue, Temperature, Pressure, Humidity]';
p = [SkyRed, SkyGreen, SkyBlue, Temperature, Pressure, PM10]';
q = [SkyRed, SkyGreen, SkyBlue, Temperature, Pressure]';
% A has R,G,B,T,P and PM levels.
...
legend("True Responses","GP Predictions",...
    "Prediction Intervals",Location="best")
%% scatter plot of gaussian process regression.
...
Out_Train = Ytrain;
Out_Validation= Ytest;
Out_TrainEstimate=predict(gpMdl,Xtrain);
Out_ValidationEstimate=predict(gpMdl,Xtest);
scatter(Ytrain,Out_TrainEstimate,'gs','filled')
sz = 50;
scatter(Ytest,Out_ValidationEstimate,sz,'r*')
hold off
...
% Requires R2020a or later
exportgraphics(f,'scatter_diagram_gaussian_process_regression.png','Resolution',300)
%print('-dpng',fn_plot);% save to a png file
%print('-depsc2',fn_plot);% save to a color eps file